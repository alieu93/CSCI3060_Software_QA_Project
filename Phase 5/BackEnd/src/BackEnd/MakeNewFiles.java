...
public class MakeNewFiles {
	...
	static String ATFFile;
	static String UAFFile;
	...
	public static boolean applyTransactions(){
		...
		if(!writeToUAF(UAFFile)){
			ErrorLog err = new ErrorLog("fatal", "Error writing to user account file.");
			return false;
		}
		if(!writeToATF(ATFFile)){
			ErrorLog err = new ErrorLog("fatal", "Error writing to available tickets file.");
			return false;
		}
	...
	public static boolean writeToUAF(String file){
	...
	public static boolean writeToATF(String file){
	...
	public static boolean logout(String line) {
		...
		String name = DTF_Line.substring(0, 14);
		...
		//Check for incorrect username, should never happen
		if(!check){
			ErrorLog err = new ErrorLog("Constraint error", "Error Logging out, invalid username", "Logout");
			return false;
		}
		return true;
	}
	...
	public static boolean sell(String line){
		...
		String eventName = sellLine.substring(0, 18);
	...
	public static boolean buy(String line){
		...
		//Check if the event name was ever found
		if(nameCheck == false){
			ErrorLog err = new ErrorLog("Constraint error", "Event does not exist.", "Buy");
			return false;
		}
